...
namespace Logics_Layer.Admin
{
    public class SliderSetting_Logic : PDBC
    {
        ...
        public string DeletePic_Sliders(int PicId)
        {
            ...
            base.Connect();
            DataTable dt = base.Select("SELECT [PicId] ,[PicUseAs] FROM [tbl_PicUse] where PicId=@P_Id && PicUseAs LIKE N'Slider'", parss);
            if (dt.Rows.Count != 0)
            {
                s += base.Script("DELETE FROM [tbl_PicUse] WHERE PicId=@P_Id && PicUseAs LIKE N'Slider'", parss);
            }
            base.DC();
            if (s == "1")
            {
        ...
        public string AddPic_Sliders(List<int> PicId)
        {
            ...
            for (int i = 0; i < PicId.Count; i++)
            {
                ...
                parss.Add(par);
                dt = base.Select("SELECT [PicId] ,[PicUseAs] FROM [tbl_PicUse] where PicId=@P_Id && PicUseAs LIKE N'Slider'", parss);
